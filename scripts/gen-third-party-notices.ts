...
/** First-party private native packages: reachable at runtime but not third-party. */
...
The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and use private \`@deepseek-ai\` workspace names. All are MIT-licensed; each directory preserves its upstream \`LICENSE\` file. Exact upstream commits and local modifications are recorded in [\`vendor/README.md\`](vendor/README.md).

...
前几篇教程通过 `--patch` overlay 加载本地插件。本教程把一项单独归属的扩展打包成可安装的**组合包**（bundle），用 `kosma plugin add` 安装进一个 **profile**，并解释决定组合后配置的层顺序。kosma 内置 workspace 是私有源码组件，不走这条分发路径。本文假设 `kosma` CLI 已安装。请先完成[插件配置](./config.md)。
...
组合包是你编写并分发的东西；profile 是用户用 `kosma --profile <name>` 启动的东西。没有东西同时是两者。
...
没有 `dsh.bundle` 声明的包仍然可以安装，但只作为普通依赖：`kosma plugin` 会打印警告，且不激活任何层。如果一个库供插件包 import，而不是供用户启用，就使用这种包格式。
...
profile manifest 从不需要手写：`kosma plugin` 负责创建和维护它。下一节展示其结果。

## 安装进 profile

`kosma plugin --profile <name> <args...>` 在 profile 目录内转发给 pnpm，因此所有 pnpm 子命令都可用。在包含 `hello-plugin` 的目录中安装该包的 checkout：

```sh
kosma plugin --profile demo add ./hello-plugin
```

首次使用会初始化 profile（`@deepseek-ai/dsh-base` 作为它的第一个组合包），pnpm 链接该 checkout，而 `kosma` 因为这个包声明了 `dsh.bundle`，把它追加进 `dsh.profile.bundles`：
...
`kosma plugin --profile demo remove dsh-hello-plugin` 会同时移除依赖和对应的层。
...
- **用户**为构建授权。pnpm ≥10 在得到显式允许之前拒绝运行 git 依赖的 `prepare` 脚本，所以第一次 `add` 会失败；`kosma` 会指出修法——把 pnpm 打印的确切包键复制进该 profile 的 `pnpm-workspace.yaml`：
...
- **发布到 npm**，在 `pnpm publish` 时构建好 `lib/`；`kosma plugin add your-package` 安装的就是预构建代码。
- **交付 tarball**：用 `pnpm pack` 打包；用户执行 `kosma plugin add ./hello-plugin-0.1.0.tgz`。

...
前几篇教程通过 `--patch` overlay 加载本地插件。本教程把一项单独归属的扩展打包成可安装的**组合包**（bundle），用 `dsh plugin add` 安装进一个 **profile**，并解释决定组合后配置的层顺序。XxAgent 内置 workspace 是私有源码组件，不走这条分发路径。本文假设 `dsh` CLI 已安装。请先完成[插件配置](./config.md)。

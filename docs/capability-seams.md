...
| `ctx.xagentBusinessSkill` | `seam` | `xagent-business-skill` | - | [`api-gateway`](../packages/api/gateway), [`tool-skill`](../packages/skill/tool-skill) | - | Uses an authenticated conversation Project Session to register non-cacheable catalogs and reauthorize exact immutable loads; private definition ownership stays on the Host. |

...
Project Business Skills are governed content rather than executable plugins. FastAPI and PostgreSQL own project-local drafts, isolated test Sessions, immutable published versions, authorization, rollback, terminal retirement, idempotency, and audit. Specialists and Managers can edit and run read-only draft tests, while only Managers can publish a tested revision or change production authorization. `@xagent/dsh-business-skill` contributes a request-scoped provider to the generic Skill registry only for authenticated Project conversation Sessions. A turn pins one authorized version and its complete tool set; every tool call receives fresh backend authorization, and only a later turn observes publication or rollback. The durable `business-skill/activated` event makes restart replay deterministic, while completed model history retains only an instruction-free use marker. `@xagent/dsh-ui-business-skill` exposes the role-gated Draft → Test → Publish → Authorize workflow without receiving credentials, opaque version keys, internal record IDs, or test Session IDs.

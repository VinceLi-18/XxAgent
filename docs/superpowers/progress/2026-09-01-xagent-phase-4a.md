...
- Browser 的标准 Tool UI 用中性状态“引用验证未通过，回答未发布”表示被拒绝的终稿 Tool result；终态 `CITATION_FAILED` 仍显示通用回合失败。入账终稿由同一 Tool UI 纯函数渲染正文、citation chip 与资料条，live 与 replay 使用同一持久事件。chip 通过 citation Remote 重新授权，成功后驱动 Workbench 可观察状态切换到资料 tab，再打开不可变 Artifact Version、精确行范围和高亮；撤权或账号切换会清空旧账号 UI 状态。
...
根 Vitest 配置不收集 `*.e2e.ts`：Task 12 的根配置命令只收集 37 个文件，结果为 36 个文件通过、1 个跳过，722 个测试通过、1 个跳过；拥有该浏览器测试的 `vitest.web.config.ts` 明确收集并通过 1 个文件、1 个测试。完整 built-Web 门禁通过 77 个文件、255 个测试，另有 2 个文件和 16 个测试按各自条件跳过。

## GIF 证据

同一次绿色完整流的 5 张等尺寸源帧覆盖登录与项目范围、中性终稿纠正状态、规范终稿与可验证 citation chip、不可变 Version/行定位，以及账号隔离。编码文件位于忽略目录 `.playwright-mcp/task12-structured-retrieval/xagent-structured-retrieval.gif`；精确尺寸、帧数、时长、字节数、SHA-256 与 demonstrated commit 记录在本地 Task 12 report。

GIF 本体与每个停留状态的解码代表帧均经过视觉检查。画面没有密码、bearer token、delegation、object key、signed URL、内部 query 或 secret 环境值。GIF、源帧和 provenance 没有发布到 assets branch、PR 或远端。

...
The checked-in keyless headless snapshot assembles the Business profile with the real Agent loop and exercises a published Skill through the existing Retrieval and Fact capability seams. The Browser acceptance uses the built XAgent Business profile, real FastAPI/PostgreSQL state, two authenticated project roles, the configured real model, and a fresh browser context; test-only transport, synthetic UI state, and credential capture are excluded.

## Remaining release work

The implementation, deterministic acceptance, current bilingual documentation, and implemented decision record are complete locally. Publication still requires pushing the final commit, publishing the inspected secret-free GIF on the dedicated assets branch, attaching it to the pull request, and waiting for the remote required checks. Those remote mutations are intentionally outside this local Task 11 execution.

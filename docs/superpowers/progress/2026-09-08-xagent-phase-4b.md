...
Review completion writes one required-on-read `fact/proposal-decided` event containing only the closed public proposal, project, field, label, terminal status, optional confirmed revision fields, and optional decision reason. Outbox delivery never calls Agent send or follow-up APIs. The Fact plugin derives ordered, unconsumed decisions solely from the durable Session log and adds them only to the next user-initiated model request. It waits for downstream streaming to begin, durably replaces the temporary notice, and flushes before yielding; downstream failure, cancellation, append failure, or flush failure leaves the decision available for exact retry. Same-Turn continuations, later Turns, and restart replay do not repeat a consumed notice.

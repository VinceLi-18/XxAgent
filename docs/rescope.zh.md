...
Cordis 框架及其基础库以源码形式 vendored 在 [`vendor/`](../vendor/README.md) 下，并使用私有 `@deepseek-ai` workspace 名。独立的内部名称将 vendored 源码图与普通上游依赖分开，并让每个应用 import 都解析到经过审阅的 checkout。本页是名字映射表；决策与影响见[改名 Agent Note](../.agents/notes/implemented/process/2026-08-10-vendor-package-rescope.md)，上游 commit 见 [`vendor/README.md`](../vendor/README.md)。

...
async def test_bootstrap_excludes_test_session_scopes_and_counts(client, started_test, existing_actor_headers, fact_project_session):
    response = await client.post("/internal/xagent/workbench/bootstrap", headers=existing_actor_headers, json={"schema_version": 1})
    assert response.status_code == 200, response.text
    assert response.json()["session_summary"]["project_counts"][str(fact_project_session.project_id)] == 1
    assert started_test[0]["session_id"] not in str(response.json()["session_scopes"])

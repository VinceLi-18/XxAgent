...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint(
            "session_id",
            "citation_id",
            name="pk_xagent_admitted_evidence",
        ),
        sa.UniqueConstraint(
            "session_id",
            "citation_id",
    ...
    for table in ("xagent_admitted_evidence", "xagent_cited_answer_evidence"):
        op.execute(f"ALTER TABLE {table} ENABLE ROW LEVEL SECURITY")
        op.execute(f"ALTER TABLE {table} FORCE ROW LEVEL SECURITY")
        op.execute(
            f"CREATE POLICY {table}_read ON {table} "
            f"FOR SELECT TO {application_role} USING ({session_read})"
        )
        op.execute(
            f"CREATE POLICY {table}_insert ON {table} "
            f"FOR INSERT TO {application_role} WITH CHECK ({session_write})"
        )
...
def downgrade() -> None:
    application_role = _application_role()
    for table in ("xagent_cited_answer_evidence", "xagent_admitted_evidence"):
        op.execute(f"REVOKE ALL PRIVILEGES ON {table} FROM {application_role}")
    ...
    op.drop_table("xagent_admitted_evidence")
    op.drop_constraint(
        "uq_artifact_text_chunk_id_index",
        "artifact_text_chunks",
        type_="unique",

 ...
 * `kosma --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `kosma --profile web -h` prints the web app's help, not this one's.
 *
...
const HELP_EXAMPLES = `
Examples:
  kosma --profile web                          boot the web profile (same as: kosma web)
  kosma --profile headless "run the tests"     answer one task, print the result, and exit
  kosma --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  kosma --profile tui --resume <session>       arguments after the launcher flags reach the app
  kosma --profile web --help                   the web app's own flags and help
  kosma plugin --profile tui add <package>     install a plugin into the tui profile
`
...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  program
    ...
    .description('kosma: boot an agent profile powered by DeepSeek Harness — an ordered stack of plugin-bundle patch layers under your own overrides.')
    ...
    .argument('[args...]', 'arguments for the booted profile\'s app (see: kosma --profile <name> --help)')
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: kosma web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  if (resolved === undefined) throw new Error('kosma: no invocation resolved')
  return resolved
}

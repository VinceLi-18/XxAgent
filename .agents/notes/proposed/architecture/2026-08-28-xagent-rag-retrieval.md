...
资料检索需要同时保存不可变版本的文本分片、1024 维向量、关键词索引、索引任务、当前搜索 generation 和与 Session 证据对应的短期收据。把这些状态放在 Host、Browser 或内存任务队列会复制 FastAPI 的 Principal、项目成员和资料 Version 授权，并让进程重启或撤权后的结果失去可验证来源。
...
`vector` 扩展必须由 PostgreSQL 部署镜像提供；没有扩展的数据库会在迁移时失败关闭。新增 worker 输入或输出列时必须同时审计 RLS policy 和列/表授权。后续检索、索引和证据入账实现必须使用这里的持久化状态，不得绕过收据或以应用层筛选替代数据库 RLS。

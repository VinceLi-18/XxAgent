...
The Landlock launcher source and package family live at `native/landlock-run`, next to the harness consumers and inside the root pnpm workspace. The [private application distribution decision](../process/2026-09-06-private-application-distribution.md) owns the shared source revision, lockfile, private package graph, and native build artifacts. The entry package owns platform selection, path resolution, probing, CLI flags, the fatal prefix, and the partial-enforcement notice while the harness maps sandbox modes to grants. Building the entry point with its binaries keeps probe parsing and launch syntax aligned.

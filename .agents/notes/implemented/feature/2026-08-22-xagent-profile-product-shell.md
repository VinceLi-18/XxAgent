...
All DSH and XAgent runtime packages are private workspaces delivered from one authorized XxAgent revision rather than npm release members. Their manifests point to exact package directories in the XxAgent repository, and the workspace-constraints gate lists each XAgent package identity explicitly. Adding a package below `packages/xagent/` therefore requires a deliberate policy entry; directory wildcards do not expand that ownership list. The [private application distribution decision](../process/2026-09-06-private-application-distribution.md) owns the repository-wide rule.
...
`packages/boot/app-boot/tests/profile.spec.ts` verifies the XAgent templates and Profile data paths; `apps/cli/tests/profile-boot.spec.ts` verifies distinct launch-time paths; and both XAgent bundle tests verify the state-row expressions and the Business capability closure. `apps/cli/tests/xagent-business-rosterless.e2e.ts` creates a Business session through the real API proxy and verifies an empty preset roster and file-skill catalog beside the two retrieval schemas. `apps/cli/tests/xagent-retrieval-runtime.e2e.ts` verifies authenticated Private and Project Sessions, request-scoped terminal admission, Code Mode absence, and citation resolution. Its Browser lane logs in at the Host origin, loads the `ClientModuleRegistry` graph served in `window.__DSH_BOOT__` through the production module system and Browser Loader, waits for the production `WebApiClient` connection to publish a Host description, and calls the generated citation Remote through the authenticated Host gateway. A test-local WebSocket adapter supplies only browser-managed Cookie and Origin headers under jsdom; the Connection, Remote, gateway, and plugin graph remain the production implementations. This lane observes the keyed Tool view only for Business, recursively rejects Host secret fields and values, and boots Developer, Web, and Headless to verify their complete absence surface.

The [RAG retrieval decision](../architecture/2026-08-28-xagent-rag-retrieval.md) continues to own retrieval data, authorization, receipt, and citation semantics. This note owns Profile composition and does not supersede that decision, the authentication and Session isolation decision, or the real-retrieval acceptance note.

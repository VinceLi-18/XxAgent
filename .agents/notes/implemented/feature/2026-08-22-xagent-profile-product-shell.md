...
Business users receive a constrained rosterless Web composition and Developer users receive an isolated local development composition without changing dsh tooling. Business gives up per-session preset selection and locally authored presets. The remaining costs are two independently persisted local states and a visible unavailable error for Business subagent API calls. Rosterless composition is a Phase 1 capability closure, not a multi-user security boundary; product-level multi-user operation still requires principal-aware authentication, authorization, session checks, and protected storage.

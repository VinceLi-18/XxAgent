...
**Use one shared `$DSH_HOME` data directory for both XAgent profiles.** Rejected because settings, credentials, sessions, attachments, and JSON storage would overlap despite different Profile responsibilities.
...
Business users receive a constrained Web composition and Developer users receive an isolated local development composition without changing dsh tooling. The cost is two independently persisted local states and a visible unavailable error for Business subagent API calls. Product-level multi-user operation still requires principal-aware authentication, authorization, session checks, and protected storage.

`packages/boot/app-boot/tests/profile.spec.ts` verifies the XAgent templates and Profile data paths; `apps/cli/tests/profile-boot.spec.ts` verifies distinct launch-time paths; and both XAgent bundle tests verify the state-row expressions and the Business capability closure. The built CLI configuration checks load both Profile compositions.

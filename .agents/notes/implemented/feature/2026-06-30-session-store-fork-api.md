...
`dsh-session` owns ordinary live-session fork policy directly on `ctx.sessions`. There is no separate `dsh-session-fork` package or `ctx.sessionFork` service: the operation has no independent event vocabulary or lifecycle, and durable work delegates to the existing session store and persistence service. `SessionPersistence.fork(sourceId, throughSequence)` is the optional authority hook for a backend that owns Session identity or authorization scope; local backends return `undefined` and retain the in-process seed path.
...
The Host composes the child from the source's logged provider, model, and reasoning target before it can run. With a local persistence backend, it creates the child through the agent registry with the selected seed and lineage. With a scope-owning remote backend, it first flushes the source, submits only the source ID and inclusive cut, and resumes the exact durable child Header returned by `SessionPersistence.fork`. The authoritative backend reauthorizes and locks the source in one transaction, assigns the child ID, and copies the exact event prefix plus backend-owned visibility, project references, and other authorization relations. Neither current workspace selection nor caller-supplied target metadata can choose the child's scope. The Host then attaches the child to the source Workspace. An attachment failure returns `workspace-attach-failed` with the already-published child id; the client reconciles that child into its summary list before surfacing the error. The Session-row action uses the last completed turn, while a message action supplies its event seq; both open the child after success, and lineage expansion makes it visible beneath the source.

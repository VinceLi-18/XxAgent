...
Landlock launcher 源码和包家族位于 `native/landlock-run`，与 harness 消费方同仓，并属于根 pnpm workspace。[私有应用发行决策](../process/2026-09-06-private-application-distribution.md)负责共享源码 revision、锁文件、私有包图和 native 构建产物。入口包拥有平台选择、路径解析、探测、CLI（命令行界面）参数、致命前缀和部分强制执行通知，而 harness 将沙箱模式映射为授权。将入口点与其二进制一起构建，使探测解析和启动语法保持对齐。

...
源码用户通过仓库脚本运行程序，而非使用已安装的 `dsh` 命令。仓库不提供原子升级切换，也不保留 staging 回滚 checkout；仓库同样不会自动集成个人源码修改。[私有应用发行决策](../process/2026-09-06-private-application-distribution.md)把授权仓库 revision 定为交付单元。未来任何 installer 或 package registry 都必须说明为何应由其管理安装和升级状态，定义恢复行为，并补充测试与用户文档，同时不得让源码运行路径依赖该机制。

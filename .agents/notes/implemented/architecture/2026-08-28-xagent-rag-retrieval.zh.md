...
同一 append 路径接受规范 `xagent-cited-answer` 时，FastAPI 会把每个首次使用的 citation ID 绑定到物理 Session 日志中更早的服务端规范检索事件。它把回答与 admission sequence 以及精确的 Artifact、Version、Index generation 和 Chunk 写入 `xagent_cited_answer_evidence`；数据库外键要求两个事件与两组不可变索引身份同时一致。该关系不依赖 `deriveMessages()`，也不依赖 admission actor 的 receipt，因此表层替换不会移除 provenance，另一名当前仍获授权的 Project 成员也能重新打开结果。Citation resolve 只接受 Session 与短 citation ID，经 Session RLS 读取该关系，再经当前 actor 的 RLS 与权限 finalizer 重新授权精确不可变 Artifact Version 和 Chunk，所以撤权后失败关闭。服务端拥有的 fork 事务只复制 admission 与回答都位于已复制事件前缀内的 provenance，并同时复制源 `visibility`、project、私有项目引用和精确事件。
...
应用角色经由 Artifact 与 Session 的现有 RLS 关系读取 Index、分片、head 和 cited-answer provenance。它只可读取、写入和更新本 actor 的收据，并且只能为 actor 可编辑的 Session 插入 provenance。worker 只获得 Index、分片、Index Job 和搜索 head 的必要权限；它不能读取账号、认证、Session、项目成员、收据或 cited-answer provenance。Browser 没有数据库角色，全部资料操作继续通过受认证的 API。
...
- 迁移启用 `vector` 与 `pg_trgm`，创建六张检索表、`next_citation_ordinal`、约束、触发器、RLS 和精确角色授权，并支持 `upgrade → downgrade → upgrade`。

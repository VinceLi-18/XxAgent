...
Windows 独占创建锁时的 `EPERM` 在现有获取期限内重试。[等待删除的文件可能拒绝新打开操作](https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-deletefile)，因此权限错误不一定表示永久拒绝。持续的 `EPERM` 在期限届满时保留原错误；竞争者不会移除锁，也不会在独占创建成功前执行操作。获取阶段的故障注入测试覆盖恢复、所有权保留、持续失败，以及非 Windows 或无关错误立即失败；settings 并发测试覆盖真实的提供方竞争。

...
`dsh-tools` owns one JSON-value schema vocabulary with two representations. `ValueSchemaSpec` is the author form for any JSON root; `ParameterSchemaSpec` is its implicit object-property-map form with per-property `required: true`. `JsonSchemaNode` is the raw wire form. Both support string, finite number, integer, boolean, null, array, object, type-correct scalar `enum`/`const`, string `pattern`, array `maxItems`/`uniqueItems`, and exact-one `oneOf`; `{ type: 'json' }` is author-only sugar for an annotation-only unconstrained raw node. A pattern is a compilable JavaScript regular-expression source, `maxItems` is a non-negative safe integer, and `uniqueItems` is boolean. Array uniqueness uses JSON structural equality rather than serialization, so equivalent objects compare equal regardless of key insertion order.
...
`InferValue<S>` and `InferArgs<P>` derive TypeScript values from the same declarations that `valueSchemaSpecToJsonSchema()` and `parameterSchemaSpecToJsonSchema()` compile. Constraints that narrow runtime membership without changing the static primitive or collection type remain present in generated TypeScript and Python schemas without changing `string`/`str` or array/list output types. Exact inference is bounded to 16 container levels and then uses `JsonValue`, preventing TypeScript's type-instantiation stack from becoming the authoring limit. `assertSupportedJsonSchema()` rejects unsupported or misplaced keywords, and `validateJsonSchemaValue()` enforces the accepted subset against the lossless `JsonValue` boundary: no `undefined`, negative zero, non-finite numbers, sparse arrays, cycles, exotic objects, functions, symbols, or other coercive values. Author compilation, raw-schema assertion, value validation, schema-to-TypeScript and schema-to-Python rendering, registry detachment, and dynamic Cordis cross-realm normalization and cloning preserve the same accepted declarations; recursive paths use explicit work stacks, so runtime nesting is limited by available memory rather than the JavaScript call stack.
...
- Accepted string and array constraints are never declaration-only: registration validates their types and placement, while value validation executes them.

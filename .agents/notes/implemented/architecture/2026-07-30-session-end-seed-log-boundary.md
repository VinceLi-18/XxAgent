...
The constructor append happens before `enter()`, so the session has no store attachment: the marker never publishes on `session/event`, exactly like the seed events before it. A persistence implementation retains this constructor-created suffix through `SessionPreparation`. The Agent Loop commits the preparation only after `session/created`, `agent/created`, and `agent/session-start` all succeed without cancellation; the provider then enqueues the suffix before any synchronous live event buffered during publication. The generic coordinator attaches its prepared suffix through the same commit/release ownership, and the XAgent FastAPI provider retains it by exact Session identity. Cancelling or rolling back at any earlier publication point releases the suffix without a durable write, and a seed already ending in the marker has no suffix to enqueue. A consumer that watches the firehose therefore never sees the boundary and must read it from the log.

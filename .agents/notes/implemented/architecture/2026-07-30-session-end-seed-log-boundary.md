...
The constructor append happens before `enter()`, so the session has no store attachment: the marker never publishes on `session/event`, exactly like the seed events before it. A persistence implementation must retain this constructor-created suffix during preparation and synchronously enqueue it at `session/created` before any later live event. The generic coordinator attaches the prepared suffix through its reservation; the XAgent FastAPI provider retains it by exact Session identity and places it on the same append queue. Cancelling or rolling back before publication releases the suffix without a durable write, and a seed already ending in the marker has no suffix to enqueue. A consumer that watches the firehose therefore never sees the boundary and must read it from the log.

...
## Persistence owns the unpublished suffix

The constructor append happens before `enter()`, so the session has no store attachment: the marker never publishes on `session/event`, exactly like the seed events before it. A persistence implementation retains this constructor-created suffix through `SessionPreparation`. The Agent Loop commits the preparation only after `session/created`, `agent/created`, and `agent/session-start` all succeed without cancellation; the provider then enqueues the suffix before any synchronous live event buffered during publication. The generic coordinator attaches its prepared suffix through the same commit/release ownership, and the XAgent FastAPI provider retains it by exact Session identity. A `session/flush` listener during publication leaves the reservation untouched; ordinary flushes can drain its suffix only after publication commits. Cancelling or rolling back at any earlier publication point releases the suffix without a durable write, and a seed already ending in the marker has no suffix to enqueue. A consumer that watches the firehose therefore never sees the boundary and must read it from the log.

Consequences for the seam: `load()` stays a pure read, with no revision bump, no `commitRepair` on a balanced log, and no durable mark left by a rejected `append`. **A successful pickup is not a pure read**, though — after complete Agent publication its first persistence checkpoint writes where nothing was written before, so a read-only or full disk fails by that checkpoint rather than while loading. That is the one cost this placement adds, and it is narrower than the load-path version's (which failed the load itself).

...
Manifest 集合由根 `pnpm-workspace.yaml` 声明的 `packages:` 成员派生，其中包括内部 Landlock package 家族，因此新增成员区域在声明当天就会被读取，而不必等谁想起来去补一份列表。许可证与 repository 元数据取自根 workspace 已安装的 pnpm store 和 package 本地链接目录；某个 package 两处都解析不到时直接失败，而不是留下空单元格。`OVERRIDES` 收录已安装 manifest 无法回答的依赖：用 Rust 构建且省略 `license` 字段的 npm binary，以及 `modelcontextprotocol/servers` 系列——该 repository 正处在 MIT 向 Apache-2.0 的重新许可过程中，实际条款按 contribution 逐条而定。运行时依赖的许可证若不在宽松清单内即为硬失败：交付 copyleft 是一项发行决策，不该被一次重新生成悄悄吸收。Vendored package 会与 `vendor/README.md` 交叉核对，出现非 MIT 即报错；`pnpm-workspace.yaml` 的 `patchedDependencies` 列入运行时表格，因为 pnpm 在安装期就会打上这些 patch——交付产物携带的是改动过的 `@earendil-works/pi-tui` 与 `node-pty`，patch 文件本身就是改动的完整记录。

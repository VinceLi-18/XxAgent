...
若每次查询都需要重新验证权限，提供方可以返回 `{ candidates, complete: true, cacheable: false }`。完整目录仍可发布，但包含它的聚合结果不会被后续查询复用。省略 `cacheable` 时，完整观测维持普通缓存行为；`complete: false` 始终禁用缓存，不受该选项影响。
...
- **失效由提供方驱动**：注册表没有 TTL，无法推断任意远程来源是否已发生变化；可变提供方需要为每次观测禁用缓存，或在来源变化时调用注册作用域内的 `invalidate()` 能力。

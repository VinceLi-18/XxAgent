...
`ctx.skills.get()` 返回完整定义且适用的调用策略通过后，每次由 agent 发起的加载都会在其作用域内恰好派发一次串行 `skill/loaded` 事件。载荷携带接收加载的确切 `Agent`、已解析的 `SkillDefinition`，以及 `model-tool` 或 `user-explicit` 形式；只有 `model-tool` 携带模型调用 ID。工具返回规范值或 pre-step 监听器添加注入消息之前，派发必须完成，因此监听器失败会阻止 skill 正文进入模型上下文。目录 `list()` 和 `snapshot()` 读取、无法解析的名称及策略拒绝的定义均不派发该事件。

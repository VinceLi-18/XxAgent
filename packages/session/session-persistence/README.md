...
| `fork(sourceId, throughSequence, operationId): Promise<SessionHeader \| undefined>` | For a backend that owns durable Session identity or authorization scope, atomically derive a server-identified child from an authorized source prefix and return its Header. `operationId` is one stable logical request identity shared by every retry, so an authoritative backend returns the same child after an unobserved committed response. Local backends return `undefined`, so the Host uses the ordinary in-process seed path. |
| `isForkRetryable(error): boolean` | Classify a provider-owned failure for the Host's single immediate fork recovery attempt. The default rejects every failure; a remote provider admits only failures safe to repeat under the same `operationId`. |
...
| `prepare(id, signal?): Promise<SessionPreparation>` | Reserve the exact unpublished Session used by resume. A coordinator reuses an earlier inspection when available and commits pending recovery. The Agent publication owner invokes the preparation commit only after Session creation, Agent creation, and session start succeed; rollback disposal releases the unpublished reservation without persisting its suffix. |
...
Each `session/event` copies its event into the session controller. The first pending event starts a fixed batching window; later events join without resetting its deadline. The configured `writeBatchMaxDelayMs` bounds this intentional wait, not event-loop, initialization, serialized-operation, or backend latency. Events admitted during a write form a new bounded batch. `session/flush` cancels the wait and is a shared quiescence barrier that drains events admitted while it runs. A flush during publication leaves an uncommitted `SessionPreparation` reservation unattached and unwritten; after publication commits it, ordinary flushes drain the attached suffix and live events. A background failure is logged once, retains the ordered batch, and pauses automatic retry; a new event starts a fresh window, while explicit flush or backend teardown retries immediately and surfaces a repeated failure.

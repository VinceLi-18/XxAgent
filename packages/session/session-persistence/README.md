...
| `fork(sourceId, throughSequence): Promise<SessionHeader \| undefined>` | For a backend that owns durable Session identity or authorization scope, atomically derive a server-identified child from an authorized source prefix and return its Header. Local backends return `undefined`, so the Host uses the ordinary in-process seed path. |

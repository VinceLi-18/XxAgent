...
Selecting a chip sends only the current Session ID and citation ID to `xagentCitation/resolve`. The Host recovers the authenticated actor, account, and permission revision, then FastAPI resolves the Session's durable cited-answer provenance and reauthorizes its exact immutable Artifact Version and Chunk for that actor. The response contains Artifact, Version, Chunk, and line identities without a URL. The browser hands only the Artifact, Version, and line range to `xagentArtifactCitationOpener`; the handoff opens the workbench's Artifacts tab before the Artifact controller re-reads detail and preview for that exact clean version.

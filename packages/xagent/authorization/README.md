...
prompt 在 edit 预检后从同一用户令牌的可见 Session 列表解析唯一 Session ID、可见性和固定项目，再把这些事实与物理连接 Principal 一起包围完整 operation。该 operation 创建的 Agent 异步工作继承冻结作用域；缺失、重复或不一致的 Session 记录均失败关闭。

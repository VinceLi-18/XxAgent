...
`ProjectBrowser` 注册到 `sidebar.workspaces`，使用 Bootstrap 的 `sessionScopes` 过滤既有 Session 列表；折叠栏只保留工作台和当前项目入口。`ContextMarker` 注册到 `conversation.context`。`WorkbenchDetails` 注册到 `shell.details`，拥有“概览／资料／协作收件箱”页签，并声明 root-scope 单 occupant 子 Slot `xagent.workbench.artifacts`；只有“资料”页签渲染该 Slot。宽屏第三栏与窄屏抽屉由通用 layout 渲染同一个组件和 store。提交上下文或创建项目期间，`WorkbenchOperationShield` 通过 `shell.overlay` 暂停产品壳交互，防止从旧范围启动新 Session。四个组件只接收 Slot 标准 props 与 inject callbacks，不访问 Cordis Context。

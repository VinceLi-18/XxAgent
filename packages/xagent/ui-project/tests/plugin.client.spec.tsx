...
describe('XAgent Project UI 插件', () => {
  it('只通过正式插槽注册，并随声明重载和插件卸载', async () => {
    ...
    expect(slots.spec('xagent.workbench.facts')).toEqual({ kind: 'single', scope: 'root' })
    ...
    expect(slots.spec('xagent.workbench.skills')).toEqual({ kind: 'single', scope: 'root' })
    expect(details.hooks.skillsAvailable.getSnapshot()).toBe(false)
    const skillsAvailability = vi.fn()
    const stopSkills = details.hooks.skillsAvailable.subscribe(skillsAvailability)
    const disposeSkills = slots.register({ name: 'xagent.workbench.skills' }, () => null)
    expect(details.hooks.skillsAvailable.getSnapshot()).toBe(true)
    await Promise.resolve()
    expect(skillsAvailability).toHaveBeenCalledOnce()
    disposeSkills()
    expect(details.hooks.skillsAvailable.getSnapshot()).toBe(false)
    await Promise.resolve()
    expect(skillsAvailability).toHaveBeenCalledTimes(2)
    stopSkills()
    expect(details.hooks.factsAvailable.getSnapshot()).toBe(false)
    const availability = vi.fn()
    const stopAvailability = details.hooks.factsAvailable.subscribe(availability)
    const disposeFact = slots.register({ name: 'xagent.workbench.facts' }, () => null)
    expect(details.hooks.factsAvailable.getSnapshot()).toBe(true)
    await Promise.resolve()
    ...
    expect(slots.spec('xagent.workbench.facts')).toBeUndefined()

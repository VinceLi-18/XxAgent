...
执行时，工具从当前 Agent 读取 Session ID，从工具运行时读取 call ID 和取消信号，再调用 Host 检索服务。服务返回的规范数据通过输出 schema 后，纯 render 函数生成模型正文，pure presentation metadata 只生成固定 kind、payload hash 和短引用标识。工具不缓存结果，也不读取 receipt registry。

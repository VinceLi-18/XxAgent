...
The first invalid draft is fully suppressed. A log-only `xagent/citation-correction` event retains its SHA-256, an at-most-8-KiB UTF-8 prefix, the stable reason, invalid IDs, and at most 64 allowed IDs; an associated plugin-origin `user/message` carries the correction instruction into normal history. The Agent retries only the exact owned `CITATION_INVALID`. A second invalid draft records only `xagent/citation-failure` and ends with a durable Chinese `CITATION_FAILED` turn error without an assistant answer or model-visible diagnostic. Citation events use closed bounded schemas at both FastAPI append and Host restore. Cancellation, revocation, account replacement, Session replacement, and disposal release no buffered answer bytes. Drafts within 64 KiB remain in memory; larger drafts spill into a private temporary directory solely to finish the authoritative SHA-256 and are removed on success, failure, or cancellation.

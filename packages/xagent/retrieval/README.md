...
The request-scoped `xagentCitation/resolve` Browser Remote accepts only the current Session ID and a persisted `[资料N]` ID. The Typert gateway supplies the authenticated actor, account, permission revision, user token, physical connection, and request cancellation; anonymous, nested, mismatched, replaced, cancelled, and disposed scopes fail closed. The Remote sends only the short ID with a fresh exact delegation. FastAPI resolves it through the cited-answer provenance committed with the Session, then reauthorizes the exact immutable Artifact Version and Chunk for the current actor. This remains independent of the Host's live message projection and the actor-specific receipt used at admission, so reload, resume, compaction, a scope-preserving fork, and another authorized Project member retain reopening while revocation closes it. The response contains only immutable Artifact, Version, Chunk, and line identities, never a storage URL, and neither side caches a locator or URL. Public failures are limited to `unauthenticated`, `session-not-found`, `citation-invalid`, and `service-unavailable`; backend codes outside that set collapse to `service-unavailable`.

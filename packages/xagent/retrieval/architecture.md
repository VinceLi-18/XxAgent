...
正式 Business 组合把该 Host 服务、模型 Consumer、远端 Session Persistence、FastAPI/PostgreSQL admission 与 citation Browser UI 串接为一条路径。检索 `tool/result` 与私有 receipt sidecar 必须在同一远端 append 中结算，后续请求才能看到证据；规范终稿只从持久 result metadata 投影实时与重放 UI。点击 citation 时，独立 Remote 从 live Session 重建持久身份并重新授权，Browser 再通过 Artifact handoff 打开“资料”页签、读取精确 clean 版本并高亮行范围。

...
import { TurnBinding } from './turn-binding.ts'
...
export class BusinessSkillRuntimePolicy {
  ...
  private readonly executing = new Map<object, { readonly promise: Promise<void>; readonly resolve: () => void }>()
  ...
  constructor(
    ...
    this.closeListeners = [
      ...
      agent.ctx.on('tools/pre-execute', async (exec, next) => {
        ...
        this.executing.set(exec, Promise.withResolvers<void>())
        if (!await this.allowed(exec.name, exec.signal)) return { kind: 'deny', reason: DENIED }
      ...
      agent.ctx.on('tools/result', (exec, result) => {
        ...
        this.executing.get(exec)?.resolve()
        this.executing.delete(exec)
      }),
      ...
      agent.ctx.on('agent/turn-stopping', ({ turn }) => {
        if (this.turn === turn) this.clear()
      }),
      agent.ctx.on('agent/error', () => { this.clear() }),
    ]
  }
   ...
   * @param callId - tool-result identity, absent for explicit user invocation.
   */
  activate(definition: SkillDefinition, version: XAgentBusinessSkillLoad, invocation: 'model-tool' | 'user-explicit', callId?: CallId): void {
    ...
    if (this.pin !== undefined) {
      ...
      if (callId !== undefined) this.pin.callIds.add(callId)
      return
    }
    ...
    if (!tools.has('skill') || tools.has('search_artifacts') !== tools.has('submit_cited_answer')
      || digest !== version.toolPolicyDigest
      || [...tools].some(name => !SAFE_TOOLS.has(name) || runtime.get(name, this.agent) === undefined)) {
      throw failure('business-skill-policy-changed')
    }
    const pin = new TurnBinding(this.agent, definition, version, this.turn)
    if (callId !== undefined) pin.callIds.add(callId)
   ...
   * @returns quiescence after the final tool result, followed by listener and history cleanup.
   */
  dispose(): Promise<void> {
    this.denied = true
    this.schemas = undefined
    this.closing ??= Promise.allSettled([...this.executing.values()].map(value => value.promise)).then(() => {

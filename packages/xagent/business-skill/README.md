...
The Host plugin requires `agents`, `sessions`, `skills`, `tools` and `systemPrompt`. Configure `backendOrigin` as the FastAPI origin, `serviceToken` as the internal Host credential, and `maxCatalogEntries` as a positive safe integer bounding the complete catalog. Required nonblank `testProvider` and `testModel` select the real model calls made by draft tests; Browser input cannot override them. Missing or invalid configuration fails at installation; oversized backend catalogs fail closed rather than truncating authorized results.

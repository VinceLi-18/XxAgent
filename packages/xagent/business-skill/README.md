...
The ignorable `business-skill/activated` event records only slug, public version, invocation form, turn and policy digest. The ordinary tool result or `skill-invocation` message preserves the exact original body. Turn stopping replaces only those admitted messages with `Business Skill <slug> v<version> was used in turn <turn>.` Later model history contains that marker; the append log retains the instructions for transcript and request reconstruction. Completion, failure and teardown clear the pin and restrictions.

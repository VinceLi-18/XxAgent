...
固定版本、目录限制和分发保护归 Agent 所有，持续到最终 `turn/end` 或 Agent 释放。物理请求取消会锁闭执行，而不移除这些保护。分发包装层在委托前重新检查固定版本及取消状态，并将物理生命周期合并到工具正文信号；审批决定不能恢复过期请求。继续执行的步骤仍使用受限目录，且不会持有或修改取消开始时仍在等待的组装数组。

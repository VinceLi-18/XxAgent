...
`@xagent/dsh-tool-fact` registers the Native-only `propose_fact` model tool for an authenticated Project Session. The Consumer captures the physical request scope when inbox work is admitted, registers only in the matching Agent's tool registry for the accepted step, and removes the registration on scope replacement, request or connection cancellation, Turn completion, Agent failure, service replacement, or plugin disposal. Anonymous and Private requests, another Session, a missing Fact service, and Code Mode expose no callable proposal schema.

...
`@xagent/dsh-tool-fact` registers the Native-only `propose_fact` model tool for an authenticated Project Session. The Consumer captures each inbox message's physical request scope and registers the schema in the matching Agent's tool registry only when the complete batch claimed before request assembly has one shared live scope. Queued messages remain independently bound across an earlier Turn's completion or failure. Request or connection cancellation, Turn completion, Agent failure, service replacement, and plugin disposal remove the active registration. Anonymous and Private requests, mixed physical scopes, another Session, a missing Fact service, and Code Mode expose no callable proposal schema.
...
Registration requires authenticated `purpose: conversation`. A `business_skill_test` Session cannot register `propose_fact`, including after service replacement or plugin reload; missing, stale and cross-Session claims cannot reuse an earlier registration. This exclusion is independent of the [draft runner's read-only policy](../business-skill/README.md).

...
Proposal receipts and Outbox identities remain in separate private registries. Session persistence reads their sequence-bounded attachments and commits them only after FastAPI acknowledges the corresponding append. Session open and `agent/pre-step` pull at most 32 Outbox rows through one owner per Session, append ordinary `fact/proposal-decided` events, and never start a Turn. The physical scope of each claimed inbox batch authorizes its complete model step; an error releases only active ownership and preserves later queued-message bindings.
...
The provider itself adds no standing prompt text or tool schema. The Project-only Consumer returns only a proposal ID and `pending` status. Outbox delivery appends `fact/proposal-decided` as a log-only, non-surface Session event. On the next user-initiated Turn, the plugin projects all undelivered decisions in event order into that model request and durably replaces the temporary notice after the downstream stream starts. Same-Turn continuations, later Turns, and replay do not repeat the notice. Receipts, tokens, and evidence content are never projected.

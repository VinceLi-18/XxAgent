...
普通 Host fork 不走通用 create。provider 先 flush 源 Session，再只提交源 ID、包含式末 sequence 和幂等键；FastAPI 重新授权并锁定源，在同一事务由服务端分配子 ID、派生 runtime Header，并复制精确事件前缀、`visibility`、`project_id`、私有 Session 项目引用和已进入该前缀的 cited-answer provenance。Host 严格校验返回 Header 后恢复该持久子 Session，因此当前工作上下文与调用方字段都不能改变 fork 范围。

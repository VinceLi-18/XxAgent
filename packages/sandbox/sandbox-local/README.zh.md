...
内部 [`@deepseek-ai/node-addon-landlock-run`](../../../native/landlock-run/packages/entry) workspace 提供平台 launcher、功能探测和 CLI 参数词汇。该提供方只负责模式到授权的映射与 runner 选择。把路径解析和探测解析保留在带版本的 binary 中，可防止约定漂移。

...
`subagents` 服务是可选注入。未注入时，非 subagent 网关方法仍可用，而 `subagent.list`、`subagent.history`、`subagent.prompt` 和 `subagent.interrupt` 均稳定返回业务错误 `{ code: 'internal', message: 'subagent service is unavailable in this deployment', details: {} }`。
...
`session.fork` 将可选事件锚点映射到该锚点处或其后的首个 `turn/end`，使消息操作可包含该消息所在的完整轮次。锚点省略或超过末尾时，选择最后一个已完成轮次；若锚点已在日志中，而其所在轮次仍开放，则返回 `fork-unavailable`，不会向较早位置裁剪。发布后的子会话会先继承源会话的种子历史、cwd、日志中最新的 `ModelSelection` 及谱系，再加入源 Workspace。一个 RPC 只拥有一个操作身份：Host 会在返回前用该身份即时重试一次经 provider 判定为瞬态的持久 fork 或精确子会话 resume，随后只会对 `EAGAIN`、`EBUSY` 或 `ETIMEDOUT` Workspace 附加重试一次，并且不会重复已成功的阶段。后续客户端 fork 是新操作并创建新子会话。如果附加到 Workspace 仍然失败，`workspace-attach-failed` 会携带已发布的子会话 id，供客户端对账。[SessionStore fork 决策](../../../.agents/notes/implemented/feature/2026-06-30-session-store-fork-api.md)记录了为何锚点要映射到该 `turn/end`。
